...
# ============================================================
# Phase 3.3A animation.
# Default output matches the HTML placeholder:
#     erdos_renyi_1960_phase3_3a.mp4
#
# Tailored idea:
#   Grow one fixed G(n,m) process through a supercritical window just above
#   the threshold. The goal is to show that once c > 1/2, the largest
#   component becomes a genuine giant of order G(c) n, while the rest of the
#   graph remains split into much smaller pieces.
# ============================================================
...
DEFAULT_OUTPUT = Path("./erdos_renyi_1960_phase3_3a.mp4")

WINDOW_C_VALUES = [0.51, 0.55, 0.60]
WINDOW_START_C = 0.49
WINDOW_END_C = 0.62
...
def collect_events_and_snapshots(
    ...
    crossed_ten_percent = False

    for idx, edge in enumerate(edges_in_order, start=1):
        g.add_edge(*edge)
        summary = summarize_graph(g)
        frac = summary.largest_size / n if n > 0 else 0.0

        if not crossed_ten_percent and frac >= 0.10:
            crossed_ten_percent = True
            events.append(PhaseEvent(idx, f"Largest component first exceeds 10% of all vertices at m = {idx:,}"))

        if idx in bench_edges:
            c_now = idx / n
            theory = giant_fraction_theory(c_now)
            snapshots[idx] = Snapshot(
                edge_index=idx,
                c_value=c_now,
                theory_fraction=theory,
                observed_largest=summary.largest_size,
                observed_second=summary.second_size,
            ...
            events.append(
                PhaseEvent(
                    idx,
                    f"Checkpoint c ≈ {c_now:.2f}: observed largest/n = {frac:.3f}, theory G(c) ≈ {theory:.3f}",
                )
            )

    if edges_in_order:
        events.append(PhaseEvent(len(edges_in_order), f"Stopped after the supercritical window at m = {len(edges_in_order):,}"))
...
def snapshot_text(current_edge_count: int, snapshots: Dict[int, Snapshot], n: int) -> str:
    ...
    for c in WINDOW_C_VALUES:
        ...
        if snap is None or current_edge_count < m:
            ...
            pieces.append(
                f"c={c:.2f}: G(c)≈{snap.theory_fraction:.3f}, obs≈{snap.observed_fraction:.3f}, second={snap.observed_second}"
            )
    return "   |   ".join(pieces)


def make_phase3_3a_animation(
    n: int,
    seed: int,
    fps: int,
    frames_per_edge: int,
    ...
    title_ax.text(
        0.5,
        0.78,
        f"Phase 3.3A — above the threshold, a giant component of order $G(c)n$ emerges (n = {n:,})",
        ha="center",
        va="center",
        fontsize=14.9,
    )
    subtitle_text = fill(
        rf"Growing one fixed $G(n,m)$ sample from $m \approx {WINDOW_START_C:.2f}n$ to $m \approx {WINDOW_END_C:.2f}n$. The checkpoints $c \approx 0.51$, $0.55$, and $0.60$ are used to show how the largest component becomes a genuine linear giant, while the second-largest piece stays comparatively small.",
        width=88,
    )
    title_ax.text(
        0.5,
    ...
    def update(frame_index: int) -> None:
        ...
        nx.draw_networkx_edges(g, pos=positions, edgelist=edge_groups["tree"], ax=graph_ax, width=1.05, alpha=0.82, edge_color=CATEGORY_EDGE_COLORS["tree"])
        if edge_groups["unicyclic"]:
            nx.draw_networkx_edges(g, pos=positions, edgelist=edge_groups["unicyclic"], ax=graph_ax, width=1.20, alpha=0.90, edge_color=CATEGORY_EDGE_COLORS["unicyclic"])
        if edge_groups["multicyclic"]:
            nx.draw_networkx_edges(g, pos=positions, edgelist=edge_groups["multicyclic"], ax=graph_ax, width=1.30, alpha=0.92, edge_color=CATEGORY_EDGE_COLORS["multicyclic"])
        if edge_groups["largest"]:
            nx.draw_networkx_edges(g, pos=positions, edgelist=edge_groups["largest"], ax=graph_ax, width=2.55, alpha=0.94, edge_color=CATEGORY_EDGE_COLORS["largest"])
        ...
        add_colored_edgecount_line(info_ax=info_ax, edge_count=edge_count, max_edges=m_end, avg_degree=avg_degree, density=density, largest_component=summary.largest_size)

        c_now = edge_count / n if n > 0 else 0.0
        theory_g = giant_fraction_theory(c_now)
        benchmark_line = fill(
            (
                f"supercritical checkpoints: c≈0.51 → {int(round(0.51 * n)):,}   |   "
                f"c≈0.55 → {int(round(0.55 * n)):,}   |   "
                f"c≈0.60 → {int(round(0.60 * n)):,}   ||   "
                f"current G(c)≈{theory_g:.3f}   |   "
                f"largest/n≈{summary.largest_size / n:.3f}   |   "
                f"second/n≈{summary.second_size / n:.3f}   |   "
        ...
        event_text = latest_event_text(edge_count, events, linger_edges=6)
        bottom_text = (
            f"Theoretical giant fraction: G(c) ≈ {theory_g:.3f}, so G(c)n ≈ {theory_g * n:.0f}. "
            f"Observed largest = {summary.largest_size}, observed second = {summary.second_size}. "
            f"Above the threshold, the key picture is that the largest component captures a positive linear fraction of all vertices. {event_text}\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create the Phase 3.3A animation showing the emergence of a giant component above the threshold."
    )
...
def main() -> None:
    args = parse_args()
    saved_to = make_phase3_3a_animation(
        n=max(20, args.n),
        seed=args.seed,
        fps=max(1, args.fps),
