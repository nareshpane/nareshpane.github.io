...
# ============================================================
# Phase 3.4B comparison animation.
# Default output matches the HTML placeholder:
#     erdos_renyi_1960_phase3_4b.mp4
#
# Tailored idea:
#   Compare several supercritical stopping points using the same node set,
#   fixed layout, and edge order. The focus is not only the giant fraction,
#   but the decay of vertices living in tree components and the relative
#   persistence of T1/T2/T3 trees.
# ============================================================
...
DEFAULT_OUTPUT = Path("./erdos_renyi_1960_phase3_4b.mp4")
...
def summarize_graph(g: nx.Graph) -> ComponentSummary:
    ...
    giant = components[0] if components else {"nodes": [], "size": 0, "kind": "none"}
    ...
    return ComponentSummary(
        ...
        giant_fraction=(giant["size"] / g.number_of_nodes()) if g.number_of_nodes() else 0.0,
    )
...
def draw_badge(ax, x: float, y: float, text: str, face: str, edge: str, txt: str = "#111111", fontsize: float = 9.5) -> None:
    ax.text(
        x, y, text, transform=ax.transAxes, ha="center", va="center", fontsize=fontsize, fontweight="bold", color=txt,
...
def shared_footer_text(specs: Sequence[PanelSpec], summaries: Sequence[ComponentSummary], n: int) -> List[str]:
    ...
    for spec, summary in zip(specs, summaries):
        ...
        line = (
            f"{spec.label}: tree vertices / n ≈ {summary.tree_vertices / n:.3f}, theory 1-G(c) ≈ {1.0 - theory:.3f}, "
            f"T1/T2/T3 = {summary.tree_counts.get(1, 0):,}/{summary.tree_counts.get(2, 0):,}/{summary.tree_counts.get(3, 0):,}."
        )
        lines.append(fill(line, width=46))
    return lines


def make_phase3_4b_animation(
    n: int,
    ...
    caption_ax.axis("off")
    caption_ax.text(0.5, 0.82, "Phase 3.4B — tree components melt into the giant as c increases", ha="center", va="center", fontsize=17, fontweight="bold")
    caption_ax.text(
        0.5,
        0.44,
        fill(
            "All three panels use the same node set, fixed layout, and edge order. Only the stopping point changes: c = 0.55, 0.70, and 1.00. This turns the qualitative picture into a direct comparison of how many tree vertices remain outside the giant.",
            width=120,
        ),
        ha="center",
        va="center",
    ...
    caption_ax.text(
        ...
        0.08,
        fill(
            "The expectation is that total tree mass decreases as c grows, while very small trees such as T1, T2, and T3 remain the most persistent among the surviving tree components.",
            width=118,
        ),
        ha="center",
        va="bottom",
    ...
    def update(frame_index: int) -> None:
        ...
        for (graph_ax, info_ax), spec in zip(panels, specs):
            ...
            info_ax.text(0.50, 0.52, f"avg degree = {summary.mean_degree:.3f}   |   giant/n = {summary.giant_fraction:.3f}   |   1-G(c) = {1.0 - theory:.3f}", transform=info_ax.transAxes, ha="center", va="center", fontsize=9.2)
            draw_badge(info_ax, 0.27, 0.28, f"tree vertices / n = {summary.tree_vertices / n:.3f}", "#edf4ff", "#1d4f91", fontsize=8.8)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create the Phase 3.4B comparison animation tracking the decay of tree mass above the threshold.")
...
def main() -> None:
    ...
    if len(c_values) != 3:
        raise ValueError("Please pass exactly three c values.")
    saved_to = make_phase3_4b_animation(
        n=max(50, args.n),
        c_values=c_values,
        seed=args.seed,
        fps=max(1, args.fps),
