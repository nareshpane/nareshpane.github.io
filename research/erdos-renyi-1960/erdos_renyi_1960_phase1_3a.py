...
# ============================================================
# Phase 1.3A animation.
# Default output matches the HTML placeholder:
#     erdos_renyi_1960_phase1_3a.mp4
#
# Idea:
#   At the threshold scale m ~ rho * n^((k-2)/(k-1)), the number of
#   k-vertex trees should behave like a Poisson random variable.
#
#   This animation accumulates many runs for one fixed n and shows the
#   empirical histogram of the number of k-trees converging toward the
#   Poisson prediction.
# ============================================================
DEFAULT_N = 1000
DEFAULT_K = 3
...
DEFAULT_OUTPUT = Path("./erdos_renyi_1960_phase1_3a.mp4")
...
def run_trials(n: int, k: int, rho: float, trials: int, seed: int) -> SimulationSummary:
...
def add_colored_runs_line(info_ax, runs_done: int, total_runs: int, m: int, n: int, k: int) -> None:
    left = TextArea(
        f"runs shown = ",
        textprops=dict(color="#111111", fontsize=10),
    )
    middle = TextArea(
        f"{runs_done:,}/{total_runs:,}",
...
def make_phase1_3a_animation(
    ...
    summary = run_trials(n=n, k=k, rho=rho, trials=trials, seed=seed)
    ...
    x_max = max(
        6,
        max(summary.counts),
        int(math.ceil(summary.predicted_lambda + 4.0 * math.sqrt(summary.predicted_lambda))),
    )
    xs = list(range(0, x_max + 1))
    poisson_values = [poisson_pmf(x, summary.predicted_lambda) for x in xs]

    fig = plt.figure(figsize=(10.6, 8.9))
    ...
    def update(frame_index: int) -> None:
        ...
        hist_ax.plot(
            ...
            markersize=5,
            color=POISSON_COLOR,
            label=rf"Poisson prediction ($\lambda={summary.predicted_lambda:.3f}$)",
        )
        ...
        hist_ax.set_title(
            rf"Phase 1.3A — at threshold, the number of $T_{summary.k}$ tree-components should look Poisson",
            fontsize=15,
            pad=30,
        )
        hist_ax.text(
            0.5,
            1.006,
            rf"Using $m = \lfloor \rho n^{{(k-2)/(k-1)}} \rfloor$ with $\rho={summary.rho:.1f}$, $k={summary.k}$, so the limiting mean is $\lambda = {summary.predicted_lambda:.3f}$.",
        ...
        add_colored_runs_line(info_ax, runs_done=runs_done, total_runs=summary.trials, m=summary.m, n=summary.n, k=summary.k)
        ...
        info_ax.text(
            0.5,
            0.52,
            rf"sample mean = {sample_mean:.3f}   |   sample variance = {sample_var:.3f}   |   predicted $\lambda$ = {summary.predicted_lambda:.3f}   |   threshold scale $m \approx n^{{{summary.alpha:.3f}}}$",
        ...
        interpretation = (
            rf"Empirical reading so far: P(count=0)={p0:.3f}, P(count=1)={p1:.3f}, P(count=2)={p2:.3f}. "
            "At threshold the count should stay finite, with many runs at 0 or 1 and fewer as the count grows."
        )
        info_ax.text(
            0.5,
            0.14,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create the Phase 1.3A animation showing that at threshold the count of k-vertex tree-components "
            "looks Poisson across many runs."
        )
    )
...
def main() -> None:
    ...
    saved_to = make_phase1_3a_animation(
        n=n,
        k=k,
        rho=rho,
        trials=trials,
