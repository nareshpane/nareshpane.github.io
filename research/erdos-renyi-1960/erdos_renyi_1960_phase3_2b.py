...
# ============================================================
# Phase 3.2B comparison animation.
# Default output matches the HTML placeholder:
#     erdos_renyi_1960_phase3_2b.mp4
#
# Tailored idea:
#   Use the same fixed node set, same layout, and same edge order, then stop
#   three panels slightly before, exactly at, and slightly after n/2. This
#   makes the threshold comparison direct: before, at, and after the critical
#   scale in one consistent picture.
# ============================================================
...
DEFAULT_OUTPUT = Path("./erdos_renyi_1960_phase3_2b.mp4")
...
def build_panel_specs(n: int, c_values: Sequence[float]) -> List[PanelSpec]:
    ...
    for c in c_values:
        specs.append(PanelSpec(c=float(c), m_target=max(1, int(round(float(c) * n))), label=f"c = {float(c):.2f}"))
    return specs
...
def shared_footer_text(specs: Sequence[PanelSpec], summaries: Sequence[ComponentSummary], n: int) -> List[str]:
    ...
    for spec, summary in zip(specs, summaries):
        line = (
            f"{spec.label}: largest = {summary.largest_size:,}, second = {summary.second_size:,}, "
            f"largest / n^(2/3) = {summary.largest_size / ref:.2f}, second / n^(2/3) = {summary.second_size / ref:.2f}, "
...
def make_phase3_2b_animation(
    ...
    caption_ax.text(
        0.5,
        0.82,
        "Phase 3.2B — slightly before, at, and slightly after $N = n/2$",
        ha="center",
        va="center",
        fontsize=17,
        fontweight="bold",
    ...
    caption_ax.text(
        ...
        fill(
            ...
            "Only the stopping point changes: c = 0.48, 0.50, and 0.52. This makes the critical transition directly comparable.",
            width=120,
        ),
        ha="center",
        va="center",
    ...
    caption_ax.text(
        ...
        0.08,
        fill(
            "The reference scale here is n^(2/3): the largest piece should become visually striking at the threshold, but medium-sized competitors still matter.",
            width=118,
        ),
        ha="center",
        va="bottom",
    ...
    def update(frame_index: int) -> None:
        ...
        for (graph_ax, info_ax), spec in zip(panels, specs):
            ...
            draw_badge(
                ...
                0.74,
                0.28,
                f"isolates = {summary.isolates:,}   |   largest / n^(2/3) = {summary.largest_size / (n ** (2.0 / 3.0)):.2f}",
                "#f6f6f6",
                "#8a8a8a",
                fontsize=8.0,
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create the Phase 3.2B comparison animation slightly before, at, and slightly after n/2."
        )
    )
...
def main() -> None:
    ...
    saved_to = make_phase3_2b_animation(
        n=max(50, args.n),
        c_values=c_values,
        seed=args.seed,
        fps=max(1, args.fps),
