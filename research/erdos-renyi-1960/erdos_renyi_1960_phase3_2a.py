...
# ============================================================
# Phase 3.2A animation.
# Default output matches the HTML placeholder:
#     erdos_renyi_1960_phase3_2a.mp4
#
# Tailored idea:
#   Grow one fixed G(n,m) process through a narrow critical window around
#   m ~ n/2. The emphasis is that the largest component is no longer merely
#   logarithmic, but is not yet a linear giant either. Medium-sized pieces
#   still compete with it, and the natural reference scale is n^(2/3).
# ============================================================
...
DEFAULT_OUTPUT = Path("./erdos_renyi_1960_phase3_2a.mp4")

WINDOW_C_VALUES = [0.48, 0.50, 0.52]
WINDOW_START_C = 0.44
WINDOW_END_C = 0.54
...
def critical_reference_n23(n: int) -> float:
...
def collect_events_and_snapshots(
    ...
    saw_multicyclic_largest = False

    for idx, edge in enumerate(edges_in_order, start=1):
        g.add_edge(*edge)
        summary = summarize_graph(g)

        if not saw_multicyclic_largest and summary.largest_kind == "multicyclic":
            saw_multicyclic_largest = True
            events.append(
                PhaseEvent(
                    idx,
                    f"Largest component first becomes multicyclic at m = {idx:,}",
                )
            )
        ...
        if idx in bench_edges:
            ...
            snapshots[idx] = Snapshot(
                edge_index=idx,
                c_value=c_now,
                reference_n23=critical_reference_n23(n),
            ...
            events.append(
                PhaseEvent(
                    idx,
                    f"Checkpoint c ≈ {c_now:.2f}: largest = {summary.largest_size:,}, second = {summary.second_size:,}",
                )
            )

    if edges_in_order:
        events.append(PhaseEvent(len(edges_in_order), f"Stopped after the post-threshold window at m = {len(edges_in_order):,}"))
...
def snapshot_text(current_edge_count: int, snapshots: Dict[int, Snapshot], n: int) -> str:
    ...
    for c in WINDOW_C_VALUES:
        ...
        if snap is None or current_edge_count < m:
            ...
            pieces.append(
                f"c={c:.2f}: n^(2/3)≈{snap.reference_n23:.0f}, largest={snap.observed_largest}, second={snap.observed_second}"
            )
    return "   |   ".join(pieces)


def make_phase3_2a_animation(
    n: int,
    seed: int,
    fps: int,
    frames_per_edge: int,
    ...
    title_ax.text(
        0.5,
        0.78,
        f"Phase 3.2A — at the threshold, the largest component jumps toward order $n^{{2/3}}$ (n = {n:,})",
        ha="center",
        va="center",
        fontsize=15.0,
    )
    subtitle_text = fill(
        rf"Growing one fixed $G(n,m)$ sample through the critical window from $m \approx {WINDOW_START_C:.2f}n$ to $m \approx {WINDOW_END_C:.2f}n$. The checkpoints $c \approx 0.48$, $0.50$, and $0.52$ are meant to show the change from just before, to at, to just after the $n/2$ threshold.",
        width=88,
    )
    title_ax.text(
        0.5,
    ...
    def update(frame_index: int) -> None:
        ...
        benchmark_line = fill(
            (
                f"critical checkpoints: c≈0.48 → {int(round(0.48 * n)):,}   |   "
                f"c≈0.50 → {int(round(0.50 * n)):,}   |   "
                f"c≈0.52 → {int(round(0.52 * n)):,}   ||   "
        ...
        event_text = latest_event_text(edge_count, events, linger_edges=5)
        bottom_text = (
            f"Reference scale: n^(2/3) ≈ {critical_reference_n23(n):.0f}. "
            f"This window should make the largest component visually striking while medium-sized competitors still survive. "
            f"{event_text}\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create the Phase 3.2A animation showing the critical-window jump toward order n^(2/3)."
        )
    )
...
def main() -> None:
    args = parse_args()
    saved_to = make_phase3_2a_animation(
        n=max(50, args.n),
        seed=args.seed,
        fps=max(1, args.fps),
