...
#     python giant_component_evolution_n100_4graphs_stage2.py
# ============================================================
DEFAULT_N = 100
DEFAULT_NUM_RUNS = 4
DEFAULT_NUM_COLS = 2
...
DEFAULT_OUTPUT = Path("./giant_component_evolution_n100_4graphs_stage2.mp4")
...
class ComponentTypeSummary:
    ...
    def other_count(self) -> int:
        return self.bicyclic_count + self.complex_count
...
def classify_component_types(g: nx.Graph) -> ComponentTypeSummary:
    ...
    In the new event of interest, an "other" component means any connected
    component that is neither a tree nor unicyclic, i.e. excess >= 2.
    """
    tree_count = 0
    unicyclic_count = 0
    bicyclic_count = 0
...
def collect_events_until(
    ...
    for idx, edge in enumerate(edges_in_order[:max_edges], start=1):
        ...
        if not first_other and summary.other_count >= 1:
            first_other = True
            events.append(AnimationEvent(idx, "First 'other' component appears (excess ≥ 2)"))

        if not second_other and summary.other_count >= 2:
            second_other = True
            events.append(AnimationEvent(idx, "A second 'other' component appears; condition now fails"))
...
def draw_badge_center(ax, x: float, y: float, label: str, value: str, face: str, edge: str) -> None:
    ax.text(
        x,
        y,
        f"{label}: {value}",
...
def make_multi_run_animation(
    ...
    fig_width = max(16.8, 8.0 * ncols)
    fig_height = max(9.2, 4.8 * nrows + 3.2)

    fig = plt.figure(figsize=(fig_width, fig_height))
    outer = fig.add_gridspec(
        nrows=nrows + 1,
        ncols=ncols,
        height_ratios=[1.0] * nrows + [0.42],
        hspace=0.11,
        wspace=0.0005,
    )
    ...
    for idx in range(nrows * ncols):
        r = idx // ncols
        c = idx % ncols
        inner = outer[r, c].subgridspec(2, 1, height_ratios=[8.8, 3.4], hspace=0.004)
    ...
    fig.suptitle(
        "Evolution Toward the 'At Most One Other Component' Regime",
        fontsize=19.5,
        fontweight="bold",
        y=0.992,
    )
    fig.text(
        0.5,
        0.962,
        f"{num_runs} independent runs, checkpoint at m = n/2 = {checkpoint_edge}, stop at m = {max_edges}",
        ha="center",
        va="center",
        fontsize=12.8,
        color="0.28",
    ...
    common_caption_1 = (
        "Event of interest: the graph consists of trees, unicyclic components, and at most one other connected component. "
        "Here, an 'other' component means any connected component with excess at least 2."
    )
    common_caption_2 = (
        "For a connected component H, excess(H) = |E(H)| - |V(H)| + 1. "
        "Tree: 0, unicyclic: 1, and 'other' means excess ≥ 2. "
        "The condition holds exactly when the number of 'other' components is at most 1."
    )
    ...
    def update(frame_index: int) -> None:
        ...
        for idx, panel in enumerate(panels):
            ...
            draw_badge_center(legend_ax, 0.20, 0.77, "Trees", str(summary.tree_count), "#dff4df", "#2e7d32")
            draw_badge_center(legend_ax, 0.43, 0.77, "Unicyclic", str(summary.unicyclic_count), "#fff4cc", "#9c7b00")
            draw_badge_center(legend_ax, 0.66, 0.77, "Other", str(summary.other_count), "#ffe2cc", "#b85c00")

            condition_face = "#dff4df" if summary.condition_holds else "#ffd9d9"
            condition_edge = "#2e7d32" if summary.condition_holds else "#b71c1c"
            condition_text = "YES" if summary.condition_holds else "NO"
            draw_badge_center(legend_ax, 0.85, 0.77, "Other ≤ 1", condition_text, condition_face, condition_edge)

            latest = latest_event_text(edge_count, run.events, linger_edges=4)
            if summary.condition_holds:
                status_text = "Condition holds: all components are trees or unicyclic, except possibly one other component"
                status_face = "#e5f6e5"
                status_edge = "#2e7d32"
                status_color = "#1b5e20"
            else:
                status_text = f"Condition fails: the graph currently has {summary.other_count} 'other' components"
                status_face = "#ffe5e5"
                status_edge = "#b71c1c"
                status_color = "#8b0000"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create a multi-panel animation of independent random-graph runs, focusing on the event that the graph consists of trees, unicyclic components, and at most one other component."
        )
    )
