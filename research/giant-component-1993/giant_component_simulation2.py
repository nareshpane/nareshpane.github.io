...
class UnionFind:
    ...
    def has_trees_unicyclic_and_at_most_one_other_component(self) -> bool:
        """
        Return True iff the graph consists of tree components, unicyclic components,
        and at most one other connected component.

        For a connected component H, define
            excess(H) = |E(H)| - |V(H)| + 1.

        Then:
          excess = 0  -> tree
          excess = 1  -> unicyclic
          excess >= 2 -> "other component"

        The desired event is: number of components with excess >= 2 is at most 1.
        """
        other_components = 0
...
def sample_unique_edges_sparse(
    ...
    In this sparse regime, rejection sampling with a Python set remains workable.
    """
...
def trial_success(
    ...
    return uf.has_trees_unicyclic_and_at_most_one_other_component()
...
def make_plot(results: List[SimulationResult], output_plot: Path) -> None:
    ...
    ax.set_title(
        r"Probability that Graph $G(n,m)$ Has Trees, Unicyclic Components, and At Most One Other Component",
        fontsize=13,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Estimate the probability that Graph G(n,m) consists of trees, "
            "unicyclic components, and at most one other component."
        )
    )
    parser.add_argument(
        "--ns",
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("./giant_component_simulation2.csv"),
        help="CSV file for numerical results. Saved in the current folder by default.",
    )
    parser.add_argument(
        "--output-plot",
        type=Path,
        default=Path("./giant_component_simulation2.png"),
        help="PNG plot file. Saved in the current folder by default.",
    )
...
def main() -> None:
    ...
    print("Asymptotic reference value: 0.995700")
